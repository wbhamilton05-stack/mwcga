...
async function main() {
  ...
  const WIN_WORDS = ["absolutely DEMOLISHED", "totally DOMINATED", "beat the living daylights out of", "humiliated — and I mean HUMILIATED —", "steamrolled"];
  ...
  if (yResults.length) {
    ...
    for (const m of yResults) {
      ...
      if (m.s1 === m.s2 && !p.adv) {
        ...
        line = `${flag(wt)} ${wt} (${wo != null ? players[wo] : 'Spoiler'}) ${pick(WIN_WORDS)} ${lt} ${flag(lt)}, ${score}.` +
  ...
  if (tGames.length) {
    ...
    for (const m of tGames) {
      ...
      addP(`• ${flag(m.t1)} ${m.t1}${o1 != null ? ` (${EMOJI[o1]} ${players[o1]})` : ''} vs ${m.t2}${o2 != null ? ` (${EMOJI[o2]} ${players[o2]})` : ''} ${flag(m.t2)}${roundTag}${stakes.length ? ' — ' + stakes.join('; ') : ''}`);
    }
  }
